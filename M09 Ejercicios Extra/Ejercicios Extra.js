...
function capToFront(string) {
   ...
   for(var i = 0; i < string.length; i++) {
      if (string[i].toUpperCase() !== string[i]) {
         nuevaString.push(string[i]);
      }
   }
   for(var e = string.length - 1; e >= 0; e--) {
      if (string[e] === string[e].toUpperCase()) {
         nuevaString.unshift(string[e]);
      }
   }
...
function deleteAbc(string) {
   ...
   // Tu código:
   var eliminar = string.replaceAll(/[a-c]/gi, '');
   return eliminar;
}
